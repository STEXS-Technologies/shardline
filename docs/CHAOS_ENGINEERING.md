...
| Object storage | Partial | `deployment_chaos` kills and partitions MinIO during writes; S3 retry, multipart and integrity tests cover normal failures; a deterministic CAS adapter commits PUT then loses its acknowledgement and proves exact-byte idempotent recovery; OCI tombstone tests replay the equivalent DELETE ambiguity | Repeat accepted-PUT response loss through a provider-backed S3 proxy/emulator; inject slow bodies, stale responses and provider-specific 5xx behavior |
