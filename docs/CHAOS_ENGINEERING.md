...
| Object storage | Covered | `deployment_chaos` kills and partitions MinIO during writes; a typed one-shot HTTP proxy proves exact-byte behavior for accepted PUT response loss, a retryable provider `503`, delayed successful range bodies and a stale-but-well-formed xorb response against real MinIO; the stale response is driven through the Xet HTTP frontend and rejected by end-to-end CAS validation before transfer bytes are exposed; Postgres-kill and MinIO-kill mid-LFS-PATCH prove durable resumable sessions survive the outage and objects complete byte-exact after recovery; netem partition during overlapping LFS PATCH repair drops the repair cleanly; Postgres-kill mid-OCI blob-upload proves the durable session survives and the blob is byte-exact after re-staging; Postgres-kill mid-S3 multipart proves the durable session survives and the object completes byte-exact after re-uploading the missing part; multipart/integrity tests and OCI tombstone DELETE replay cover the other normal and ambiguous paths | Extend the provider/version matrix as external deployments contribute evidence |
...
The same workflow is a required dependency of the tag-release workflow. Release runs
use a bounded 600-second deterministic campaign plus the deployment, resource-pressure,
