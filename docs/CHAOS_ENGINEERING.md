...
| Object storage | Partial | `deployment_chaos` kills and partitions MinIO during writes; a typed one-shot HTTP proxy proves exact-byte behavior for accepted PUT response loss, a retryable provider `503`, and delayed successful range-response bodies against real MinIO; multipart/integrity tests and OCI tombstone DELETE replay cover the other normal and ambiguous paths | Inject a stale-but-well-formed provider read and require end-to-end CAS verification to reject it |
