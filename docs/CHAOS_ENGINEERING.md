...
| Time | Partial | GC injects forward clock jumps and fails closed without stamping or deleting; Postgres-backed lifecycle repair takes shared retention time from PostgreSQL rather than replica clocks and verifies epoch results across extreme session timezones; token expiry and corrupted/future timestamp boundaries are deterministic | Inject live multi-node verifier clock skew around token issuance/expiry; fencing itself is epoch/lock based and clock-independent |
...
| Upgrade | Covered | Same-binary role replacement, migration up/down checks and typed pre/post-commit migration interruptions; a scheduled deployment drill builds tagged `v1.6.0` and the current commit as separate binaries, runs them concurrently against shared Postgres/S3, rolls both nodes forward one at a time, rolls one back, and requires bidirectional exact-byte reads and writes throughout | Advance the tagged N-1 input and compatibility assertions for every release |
