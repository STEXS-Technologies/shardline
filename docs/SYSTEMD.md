...
Use `rediss://` and mount PEM files for a TLS-protected Redis cache.
The optional CA file is `SHARDLINE_RECONSTRUCTION_CACHE_REDIS_TLS_CA_FILE`; mTLS
additionally requires both `SHARDLINE_RECONSTRUCTION_CACHE_REDIS_TLS_CLIENT_CERT_FILE`
and `SHARDLINE_RECONSTRUCTION_CACHE_REDIS_TLS_CLIENT_KEY_FILE`. See
