...
| S3 frontend (protocol) | **Beta** | S3-compatible object API — Put/Get(+Range)/Head/Delete, conditional requests (If-Match/If-None-Match), CopyObject, multipart upload (create/part/complete/abort), ListObjectsV2, DeleteObjects, ListBuckets, and bucket stubs; SigV4 access-key=token auth plus a Bearer auth bridge; index-backed listing with zero object-store reads; validated against real clients (`mc`, pyarrow 25) in the real-client e2e suite and security-audited (`feat/s3-frontend`); conformance-tested but limited production evidence |
...
- S3 frontend: object `PUT`, `GET` (with `Range`/`Content-Range`), `HEAD`, and `DELETE`;
  ...
  `AbortMultipartUpload`); `HeadBucket`/`GetBucketLocation`/`CreateBucket` stubs; and
  `ListObjectsV2` (`prefix`/`delimiter`/`max-keys`/`continuation-token`/`start-after`),
  with the SigV4 access-key and Bearer auth bridges
