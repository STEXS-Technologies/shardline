...
Shardline is designed to fit existing Git workflows.

Clients should keep using normal Git operations.
Configuration decides how the client speaks the native Xet CAS path.
...
Configure the client or filter process to target the Shardline CAS base URL.

The exact client flags depend on the Xet-compatible implementation, but the expected
