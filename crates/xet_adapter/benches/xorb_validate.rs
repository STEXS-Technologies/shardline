...
fn build_xorb(num_chunks: usize, chunk_size: usize) -> (Vec<u8>, ShardlineHash) {
    ...
    for i in 0..num_chunks {
        ...
        chunk_hashes_and_sizes.push((chunk_hash, u64::try_from(chunk_size).expect("size fits in u64")));
    }

    let xorb_hash = xorb_hash(&chunk_hashes_and_sizes);

    let SerializedXorbObject { serialized_data, .. } =
    ...
    let expected_hash = ShardlineHash::from_bytes(
        xorb_hash.as_bytes().try_into().expect("hash is 32 bytes"),
    );
...
fn bench_decode_serialized_xorb_chunks(c: &mut Criterion) {
    ...
    for chunk_count in [1, 4, 16, 64] {
        ...
        let validated = validate_serialized_xorb(&mut reader, hash).expect("valid xorb");
...
criterion_group!(benches, bench_validate_serialized_xorb, bench_decode_serialized_xorb_chunks);
