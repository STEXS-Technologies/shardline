...
pub fn build_reconstruction_response(
...
) -> Result<FileReconstructionResponse, XetAdapterError> {
    record.validate_reconstruction_plan()?;
    if record.total_bytes == 0 {
        if requested_range.is_some() {
            return Err(XetAdapterError::RangeNotSatisfiable);
        }
    ...
    let effective_range = match requested_range {
        ...
        None => {
            ...
            ByteRange::new(0, end_inclusive).map_err(|_error| XetAdapterError::Overflow)?
        }
    };
    ...
    for chunk in &record.chunks {
        let Some(chunk_end_inclusive) = chunk
            ...
            return Err(XetAdapterError::Overflow);
        };
...
pub fn build_batch_reconstruction_response(
...
mod tests {
    ...
    fn response_rejects_requested_range_past_file_length() {
        ...
        assert!(matches!(
            response,
            Err(XetAdapterError::RangeNotSatisfiable)
        ));
    }
