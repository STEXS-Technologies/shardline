...
impl ProtocolMetrics {
    ...
    pub fn new(registry: &Registry) -> Self {
        ...
        let oci_registry_token_requests = IntCounter::new("shardline_oci_registry_token_requests_total", "OCI registry token requests").expect("prometheus metric names are static constants");
        let oci_registry_token_rate_limited = IntCounter::new("shardline_oci_registry_token_rate_limited_total", "OCI registry token requests rate limited").expect("prometheus metric names are static constants");
        let oci_registry_token_active = IntGauge::new("shardline_oci_registry_token_active_requests", "Active OCI registry token requests").expect("prometheus metric names are static constants");
        ...
        registry.register(Box::new(oci_registry_token_requests.clone())).ok();
        registry.register(Box::new(oci_registry_token_rate_limited.clone())).ok();
        registry.register(Box::new(oci_registry_token_active.clone())).ok();

        Self { lfs_uploads, lfs_downloads, oci_uploads, oci_downloads, hub_api_requests, hub_api_commits, hub_api_file_uploads, hub_api_file_downloads, oci_registry_token_requests, oci_registry_token_rate_limited, oci_registry_token_active }
    }
