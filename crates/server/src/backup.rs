...
mod tests {
    ...
    // ── write_backup_manifest with Postgres config ──────────────────────

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn write_backup_manifest_with_postgres_config_reports_error() {
        // When the config has an index_postgres_url but no real PG is running,
