...
async fn stats_scoped_reports_only_the_requested_repository() {
    let result = exercise_stats_scoped_reports_only_the_requested_repository().await;
    let error = result.as_ref().err().map(ToString::to_string);
    assert!(
        result.is_ok(),
        "postgres scoped stats flow failed: {error:?}"
    );
}

async fn exercise_stats_scoped_reports_only_the_requested_repository() -> Result<(), Box<dyn Error>>
{
    let Some(base_url) = env_var("DATABASE_URL").ok() else {
        return Ok(());
    };

    let database_name = format!("shardline_stats_scoped_{}", process::id());
    let admin_url = database_url_for(&base_url, "postgres")?;
    let admin_pool = PgPoolOptions::new()
        .max_connections(1)
        .connect(&admin_url)
    ...
    backend
        .record_store
        .write_latest_record(&file_record(&scope_a, "a-hash"))
        .await?;
    backend
        .record_store
        .write_latest_record(&file_record(&scope_b, "b-hash"))
        .await?;

    let scoped_a = backend.stats_scoped(&scope_a).await?;
    let scoped_b = backend.stats_scoped(&scope_b).await?;
    let whole = backend.stats().await?;
