...
async fn drill_extreme_4_postgres_kill_mid_transaction() {
    ...
    if put_status != 200 {
        ...
        eprintln!(
            "SKIP: drill_extreme_4_postgres_kill_mid_transaction — PG metadata path not \
             functional (seed PUT -> {put_status}, body={body:?}); likely missing schema \
             migrations (server does not self-migrate). Reported as an open item."
        );
        return;
    }
